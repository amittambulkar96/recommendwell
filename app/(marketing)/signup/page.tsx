...
export default function LoginPage() {
  ...
  async function onSubmit(data: SignUpFormData) {
    ...
    try {
      ...
      if (signUpError) {
        throw new Error(signUpError.message || "Something went wrong.");
      }

      if (!session?.user) throw new Error("User could not sign up");
  ...
  return (
    <section className="bg-linear-to-b from-muted/50 to-background flex min-h-screen px-4 py-16 md:py-32">
      ...
      >
        ...
        <p className="text-muted-foreground/50 text-xs px-6">
